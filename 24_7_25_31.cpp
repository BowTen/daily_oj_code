...
//	int n, m;
//	cin >> n >> m;
//	vector<int>par(n+5), p(n+5), siz(n+5, 1), q(n+5);
...
//		g[par[p[i]]].insert(i);
//	}
//	for(int i = n;i >= 2;i--) siz[par[i]] += siz[i];

//	auto check = [&](int i) -> int {
//		if(g[i].empty()) return 1;
//		return g[i].empty() ? 1 : (q[i]+1 == *g[i].begin() && q[i]+siz[i] == *--g[i].end() + siz[p[*--g[i].end()]]);
//	};

//	set<int>ac;
//	for(int i = 1;i <= n;i++){
//		if(check(i)) ac.insert(i);
//	}
...
//		p[q[x]] = x; p[q[y]] = y;
//		for(auto v : st) if(v){
//			if(check(v)) ac.insert(v);
//			else ac.erase(v);
//		}

//		if(ac.size() == n) cout << "YES\n";
//		else cout << "NO\n";
//	}
//}	
...
//				st[top-1][0] += st[top][0] - mx;
//				top--;	
//				mx = 0;
//			}else if(st[top][0] >= st[top-1][0]){
//				mx = max(mx, st[top-1][0]);
//				st[top-1] = st[top];
//				top--;
//			}else break;
//		}
...
//		cin >> a[i];
//		if(vis[a[i]]) a[i] = 0;
//		if(st.size() && *--st.end() > a[i]) {
//			for(auto v : st) if(v && !vis[v]){
//				vis[v] = 1;
//				ans++;
//			}
//			st.clear();
...
//	auto check = [&](int i, int x) -> int {
//		int cnt = 0;
//		if(a[i] < x) cnt++;
//		if(a[i+1] < x) cnt++;
//		cnt += tr1.query(1, (x-1)/2);
//		cnt += tr2.query(1, (x-1)/2);
//		return cnt <= k;
//	};
...
//	//		for(int k = j+1;k <= n;k++){
//	//			int s = i+j+k;
//	//			int lc = lcm(lcm(i, j), k);
//	//			if(lc != k && lc < s){
//	//				//cerr << i << ' ' << j << ' ' << k << ' ' << s << ' ' << lc << endl;
//	//				cerr << lc/i << ' ' << lc/j << ' ' << lc/k << endl;
//	//				cnt++;
//	//			}
//	//		}
//	//	}
...
//	vector<int>fac(r+5);
//	for(int i = l;i <= r;i++){
//		for(int j = i;j <= r;j += i) fac[j]++;
//	}

//	int d1 = 0, d2 = 0;
//	int ans = 0;
//	for(int i = l+2;i <= r;i++) ans += C2(i-l);
//	//cerr << ans << endl;
//	for(int i = l;i <= r;i++) if(fac[i]-1 >= 2){
//		ans -= C2(fac[i]-1);
//		//d1 += C2(fac[i]-1);
//		//cerr << i << ' ' << fac[i] << endl;
//	}
//	//cerr << ans << endl;
//	//for(int i = l;i <= r;i++){
//	//	if(i*4 <= r && ((i*4) % 3 == 0) && ((i*4) % 2 == 0)) ans--;
//	//	if(i*5 <= r && ((i*5) % 3 == 0) && ((i*5) % 2 == 0)) ans--;
//	//}
//	for(int i = l;i/2 <= r;i++) {
//		if(i % 4 == 0 && i % 3 == 0 && i/4 >= l){
//			ans--;
//			d1++;
//			//cerr << i/4 << ' ' << i/3 << ' ' << i/2 << ' ' << i << endl;
//		}
//		if(i % 5 == 0 && i % 3 == 0 && i % 2 == 0 && i/5 >= l){
//			ans--;
//			d1++;
//			//cerr << i/5 << ' ' << i/3 << ' ' << i/2 << ' ' << i << endl;
//		}
//	}
//	//cerr << endl;
...
//	int n, m;
//	cin >> n >> m;
//	int s1 = m*(m+1)/2 + n*(m+n*m)/2 - m;
//	int s2 = n*(1+n*m-m+1)/2 + m*(n*m+n*m-m+1)/2;
//	cout << min(s1,s2) << endl;
//}	
...
//		for(int i = n-1;i >= 0;i--){
//			a[i] += f;
//			if(i == n-1) b[i] = (11 - a[i]) % 10;
//			else b[i] = (10 - a[i]) % 10;
//			f = (b[i]+a[i])/10;
//		}
//	}

//	for(auto c : b) cout << (char)(c+'0');
//	cout << endl;
//}	
...
//	vector<int>f(n+5), t(n+5);
//	for(int i = 1;i <= n;i++){
//		f[i] = max(f[i-1], (pre[i]+i-1)/i);
//		int d = f[i]*i - pre[i];
//		t[i] = f[i] + max(0ll, (pre[n]-pre[i]-d+i-1)/i);
//	}
...
//	cin >> a >> b;
//	multiset<char>s[2];
//	for(auto c : a) s[0].insert(c);
//	for(auto c : b) s[1].insert(c);

//	int ls = -1, cnt = 0;
//	while(s[0].size() && s[1].size()){
//		int mn = 0;
//		if(*s[1].begin() < *s[0].begin()) mn = 1;
//		if(mn == ls && cnt == k) mn ^= 1;
//		if(mn != ls) cnt = 0;
//		cnt++;
//		ls = mn;
//		c.push_back(*s[mn].begin());
//		s[mn].erase(s[mn].begin());
//	}
...
//	int n;
//	cin >> n;
//	vector<int>a(n+5), vis(n+5), b(n+5);
//	for(int i = 1;i <= n;i++) cin >> a[i];
//	if(n == 1){
//		cout << -1 << endl;
//		return;
//	}
//	for(int i = 1;i <= n;i++){
//		for(int j = 1;j <= n;j++) if(!vis[j] && a[i] != j) {
//			b[i] = j;
//			vis[j] = 1;
//			break;
//		}
...
//	pre[0] = pre[1] = suf[0] = suf[1] = vector<int>(n+m+5);
//	for(int i = 1;i <= n;i++){
//		cin >> mp[i];
//		mp[i] = ' ' + mp[i] + ' ';
//		for(int j = 1;j <= m;j++) if(mp[i][j] == 'B') {
//			for(int k = 0;k < 2;k++){
//				int id = i + (k ? m-j+1 : j);
//				pre[k][id]++;
//				suf[k][id]++;
...
//	for(int k = 0;k < 2;k++){
//		for(int i = 1;i <= n+m;i++) pre[k][i] += pre[k][i-1];
//		for(int i = n+m;i >= 1;i--) suf[k][i] += suf[k][i+1];
//	}

//	//for(int k = 0;k < 2;k++){
//	//	cerr << k << " : \n";
//	//	for(int i = 2;i <= n+m;i++) cerr << pre[k][i] << ' ';
//	//	cerr << endl;
//	//	for(int i = 2;i <= n+m;i++) cerr << suf[k][i] << ' ';
//	//	cerr << endl;
//	//}
...
//			for(int k = 0;k < 2;k++){
//				int pos = i + (k ? m-j+1 : j);
//				//if(i == 5 && j == 5) cerr << pos << endl;

//				int l = 2, r = pos-1;
//				while(l <= r){
//					int mid = l + r >> 1;
//					if(pre[k][mid]) r = mid - 1;
//					else l = mid + 1;
//				}
//				dis[i][j] = max(dis[i][j], pos-l);
				
//				l = pos+1, r = n+m;
//				while(l <= r){
//					int mid = l + r >> 1;
//					if(suf[k][mid]) l = mid + 1;
//					else r = mid - 1;
//				}
//				dis[i][j] = max(dis[i][j], r-pos);
//			}

//			//cerr << dis[i][j] << ' ';
//			//if(dis[i][j] < 10) cerr << ' ';
//			if(dis[i][j] < dis[x][y]) x = i, y = j;
//		}
//		//cerr << endl;
//	}
//	cout << x << ' ' << y << endl;
...
//	int n;
//	cin >> n;
//	vector<int>a(n+5), vis(n+5), fa(n+5);
//	iota(all(fa), 0);
//	int o = 0;
//	for(int i = 1;i <= n;i++){
//		cin >> a[i];
...
//	for(int j = 1;j <= n;j++){
//		for(int i = 1;i <= n;i++) if(!vis[i] && (o & a[i])) {
//			vis[i] = 1;
//			o |= a[i];
//		}
//	}
...
//		for(int j = i+1;j <= n;j++) if(!vis[j] && (a[i] & a[j])) {
//			merg(i, j);
//		}
//	}

//	int cnt = 0;
//	for(int i = 1;i <= n;i++) if(!vis[i] && find(find, i) == i){
//		cnt++;
//		a[i]++;
//	}
//	cout << cnt << endl;
...
//	auto check = [&](int x, int val) -> int {
//		set<int>st;
//		int dt = 0;
//		for(int i = 1;i <= n;i++) if(i != x && find(find, x) == find(find, i)) {
//			if((a[i]&a[x]) == 0 && (a[i]&val)){
//				dt += bit(a[i]&val);
//				if(bit(a[i]&val) == du[i]) return 0;
//			}
//		}
//		if(dt && dt == du[x]) return 0;
//		for(int i = 1;i <= n;i++){
...
//	//check(1, a[1]-1);
//	//a[4]--;
//	//if(check(4, a[4]+1)) return;
//	for(int i = 1;i <= n;i++){
//		//cerr << i << endl;
//		a[i]++;
//		if(check(i, a[i]-1)) return ;
//		a[i] -= 2;
//		if(check(i, a[i]+1)) return ;
//		a[i]++;
//	}
