...
//	while(k--){
//		cin >> ret;
//		int xid = f3(ret)^a[x], yid = f4(ret)^a[y];
//		for(int i = 1;i <= n;i++) if(xid == a[i]) {x = i; break;}
//		for(int i = 1;i <= n;i++) if(yid == a[i]) {y = i; break;}
//		cout << y << ' ' << x << endl;
//	}
//}
...
//public:
//	Tree(int n, vector<vector<pair<int,int>>>&f) : c(n*4+5), s(n*4+5), f(f), tag(n*4+5), fg(n*4+5) {
//		build(1, 1, n);
//	}
...
//		if(ql <= l && r <= qr && c[id]) {
//			int d = prev(upper_bound(all(f[c[id]]), pair<int,int>(y,-inf)))->second;
//			c[id] = x;
//			d -= prev(lower_bound(all(f[c[id]]), pair<int,int>(y,-inf)))->second;
//			settag(id, d, c[id]);
//			//cerr << l << ' ' << r << ' ' << d << ' ' << c[id] << ' ' << tag[id] << endl;
//			return;
//		}
//		if(fg[id]) down(id);
//		int mid = l + r >> 1;
...
//	int query(int id, int l, int r, int x){
//		if(l == r){
//			//cerr << l << ' ' << c[id] << ' ' << f[c[id]].back().second << ' ' << s[id] << endl;
//			return s[id] + f[c[id]].back().second;
//		}
//		if(fg[id]) down(id);
//		int mid = l + r >> 1;
//		if(x <= mid) return query(ls, l, mid, x);
...
//	int n, q;
//	cin >> n >> q;
//	vector<vector<pair<int,int>>>f(n+5, vector<pair<int,int>>(1,{0,0}));
//	Tree tr(n, f);
...
//		}else if(s[0] == 'A'){
//			cin >> c >> x;
//			f[c].push_back({i, f[c].back().second+x});
//		}else{
//			cin >> x;
...
//	int n, tot = 0;
//	cin >> n;
//	unordered_map<int,int>last, mp;
//	vector<vector<int>>g(200005);
//	vector<int>c(200005), siz(200005);
//	for(int i = 1;i <= n;i++){
//		int x;
//		cin >> siz[i];
...
//			mp[x]++;
//			++tot;
//			if(j % 2 == 0) {g[tot].push_back(tot-1), g[tot-1].push_back(tot);}
//			if(last[x]){
//				g[last[x]].push_back(tot);
//				g[tot].push_back(last[x]);
//				last[x] = 0;
//			}else{
//				last[x] = tot;
//			}
...
//		for(L = 1;L <= i;L = R+1){
//			R = i / (i / L);
//			(f[i/L] += (pre[R]-pre[L-1]) * v[i] % mod) %= mod;
//		}
//		(f[0] += (pre[N-1]-pre[i]) * v[i] % mod) %= mod;
//	}

//	int ans = 0;
//	for(int i = 0; i <= t;i++){
//		(ans += f[i] * f[t-i] % mod) %= mod;
//	}
...
//	for(int i = 1;i <= m;i++) if(b[i] != 1) {cout << "-1\n"; return;}
...
//			(f[k] += (pre[min(N-1, j+i-1)] - pre[j-1]) * v[i] % mod) %= mod;
//		}
//		(f[0] += pre[i-1] * v[i] % mod) %= mod;
//	}
...
//	int n;
//	cin >> n;
//	vector<int>a(n+5), b(n+5), f(n+5), pa(n+5), pb(n+5);
//	iota(all(pa), 0);
//	iota(all(pb), 0);
//	for(int i = 1;i <= n;i++) cin >> a[i];
//	for(int i = 1;i <= n;i++) cin >> b[i];
//	//for(int i = 1;i <= n;i++) a[i] = rand() % 10;
//	//for(int i = 1;i <= n;i++) b[i] = rand() % 10;
//	auto aa = a;
//	auto bb = b;
//	a[0] = b[0] = inf;
...
//	f[1] = b[1] >= a[1];
//	int R1 = 1, R2 = 1, ans = f[1];
//	for(int i = 2;i <= n;i++){
//		a[++R1] = a[i]; b[++R2] = b[i];
//		while(a[i] >= a[R1-1]) R1--;
//		while(b[i] >= b[R2-1]) R2--;
//		a[R1] = a[i]; b[R2] = b[i]; pa[R1] = i; pb[R2] = i;
//		if(pa[R1-1] < pb[R2-1]){
//			f[i] = f[pa[R1-1]];
//			int l = 1, r = R2;
//			while(l <= r){
...
//				else r = mid - 1;
//			}
//			f[i] += max(0ll, pb[r] - pa[R1-1]);
//			//int p = (b.rend()-1) - lower_bound(b.rend()-1-R2, b.rend(), a[i]);
//			//if(pb[p] > pa[R1-1]) f[i] += pb[p]-pa[R1-1];
//		}else{
//			f[i] = f[pb[R2-1]];
//			int l = 1, r = R1;
//			while(l <= r){
...
//				else r = mid - 1;
//			}
//			f[i] += min(i-pb[R2-1], i-pa[r]);
//			//int p = (a.rend()-1) - upper_bound(a.rend()-1-R1, a.rend(), b[i]);
//			//f[i] += i-max(pa[p], pb[R2-1]);
//		}
//		cerr << f[i] << endl;
//		ans += f[i];
//	}
...
//	int n;
//	cin >> n;
//	vector<int>a(n+5), b(n+5), l(n+5), r(n+5);
//	for(int i = 1;i <= n;i++) cin >> a[i];
//	for(int i = 1;i <= n;i++) cin >> b[i];
//	stack<int>st;
//	st.push(0);
//	a[0] = b[0] = a[n+1] = b[n+1] = inf;
//	for(int i = 1;i <= n;i++){
//		while(b[i] >= max(a[st.top()], b[st.top()])) st.pop();
//		l[i] = st.top();
//		st.push(i);
//	}
...
//	int n, m, p, k, a, b, c;
//	cin >> n >> m;
//	vector<int>fa(n+2), ew, w(m+5), cw(m+5), idx(m);
...
//	}
//	sort(all(ew));
//	ew.erase(unique(all(ew)), ew.end());

//	vector<int>L;
//	L.push_back(-1);
//	for(int i = 0;i < ew.size();i++){
//		L.push_back(ew[i]);
//		for(int j = i+1;j < ew.size();j++)
//			L.push_back((ew[i]+ew[j]+1) / 2);
//	}
//	sort(all(L));
...
//	vector<int>d(L.size()+5), a0(L.size()+5);
//	for(int i = 0;i < L.size();i++){
//		for(int j = 1;j <= m;j++) cw[j] = abs(w[j] - L[i]);
//		iota(all(fa), 0);
//		iota(all(idx), 1);
//		sort(all(idx), [&](int i, int j) -> int {
//			if(cw[i] == cw[j]) return w[i] > w[j];
//			return cw[i] < cw[j];
//		});
//		for(auto id : idx) {
//			auto [u, v, s] = E[id];
...
//	cin >> p >> k >> a >> b >> c;
//	vector<int>q(k+5);
//	for(int i = 1;i <= p;i++) cin >> q[i];
//	for(int i = p+1;i <= k;i++) q[i] = (q[i-1] * a + b) % c;

//	int ans = 0;

//	for(int i = 1;i <= k;i++){
//		int id = upper_bound(all(L), q[i]) - L.begin() - 1;
//		ans ^= (a0[id] + (q[i] - L[id]) * d[id]);
//	}
...
//	for(int i = 1;i <= n;i++){
//		ans += abs(a[i]-b[i]);
//		int d, l = min(a[i],b[i]), r = max(a[i],b[i]);
//		if(b[n+1] < l) d = l-b[n+1];
//		else if(b[n+1] > r) d = b[n+1]-r;
//		else d = 0;
//		mn = min(mn, d);
//	}
//	ans += mn + 1;
...
//	int n, m;
//	cin >> n >> m;
//	vector<int>a(n+5+m), b(n+m+5), c(n+m+5), prec(n+m+5), prea(n+m+5), preb(n+m+5), as(n+m+5), bs(n+m+5);
//	for(int i = 1;i <= n+m+1;i++) cin >> a[i];
//	for(int i = 1;i <= n+m+1;i++) {
//		cin >> b[i];
//		c[i] = max(a[i], b[i]);
//		prec[i] = prec[i-1]+c[i];
//		prea[i] = prea[i-1] + (a[i] == c[i]);
//		preb[i] = preb[i-1] + (b[i] == c[i]);
//		as[i] = as[i-1] + a[i];
//		bs[i] = bs[i-1] + b[i];
//	}
...
//			node ret;
//			ret.sl = max(sl, s+e.sl);
//			ret.sr = max(e.sr, e.s+sr);
//			ret.s = s + e.s;
//			ret.sm = max({sm, e.sm, sr+e.sl});
//			return ret;
//		}
//	};
//	vector<int>&a;
//	vector<node>tr;
//public:
//	Tree(int n, vector<int>&a) : tr(n*4+5), a(a) {
...
//	void build(int id, int l, int r){
//		if(l == r){
//			tr[id].sl = tr[id].sr = tr[id].sm = max(0ll, a[l]);
//			tr[id].s = a[l];
//			return;
//		}
//		int mid = l + r >> 1;
