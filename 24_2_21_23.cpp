...
//     int n;
//     cin >> n;
//     vector<int>p(n+5), pos(n+5), L(n+5), R(n+5);
//     for(int i = 1;i <= n;i++){
//         cin >> p[i];
//         pos[p[i]] = i;
//     }
...
//                 int nd = p[i] - p[j];
//                 if(pos[nd] < i && pos[nd] > L[i]) ans++;
//             }
//         }else{ // L
//             for(int j = L[i] + 1;j < i;j++){
//                 int nd = p[i] - p[j];
//                 if(pos[nd] > i && pos[nd] < R[i]) ans++;
//             }
//         }
//     }
...
//     int n;
//     cin >> n;
//     vector<int>a(n+5), f(n+5, inf);
//     for(int i = 1;i <= n;i++) cin >> a[i];
//     f[0] = 0;
//     for(int i = 1;i <= n;i++){
//         int l = max(1ll, i-a[i]+1);
        
//         for(int j = l-1;j <= i;j++) f[i] = min(f[i], f[j]+1);
//         for(int j = 1;j < i;j++) if(j+a[j]-1 >= i){
//             f[i] = min(f[i], f[j-1] + 1);
//             for(int k = j+1;k < i;k++) if(k-a[k]+1 <= j){
//                 f[i] = min(f[max(0ll, k-a[k])] + 2, f[i]);
//             }
//         }
//     }
...
//         if(vec.size()) cout << "? " << mx << ' ' << vec[0] << ' ' << mx << ' ' << i << endl;
//         else cout << "? " << mx << ' ' << mx << ' ' << mx << ' ' << i << endl;
//         cin >> c;
...
//     int lsum = 0, rsum = 0;
//     vector<int>pre(n+5), suf(n+5), lpos, rpos;
//     for(int i = 1;i <= n;i++) {
//         if(s[i] == '>'){
//             pre[i] += n-i+1;
//             rpos.push_back(i);
...
//         int ans = 0;
//         while(lpos[lp] <= i) lp++;
//         while(rpos[rp+1] < i) rp++;

//         if(s[i] == '<') lsum--;
//         if(s[i-1] == '>') rsum++;

//         if(lsum == rsum){
//             if(rp >= 0) ans += (pre[rpos[rp]] - lsum*(n-i+1)) * 2;
//             ans += (suf[lpos[lp]] - lsum*(i)) * 2;    
//             if(s[i] == '>'){
//                 ans += n-i+1;
//             }else{
//                 ans += i;
//             }
//         }else if(lsum > rsum){
//             if(rp >= 0) ans += (pre[rpos[rp]] - rsum*(n-i+1)) * 2;
//             if(s[i] == '<') ans += (suf[lpos[lp]] - suf[lpos[lp+rsum]] - rsum*(i)) * 2;    
//             else ans += (suf[lpos[lp]] - suf[lpos[lp+rsum+1]] - (rsum+1)*(i)) * 2;    
//             ans += i;
//         }else{
//             ans += (suf[lpos[lp]] - lsum*(i)) * 2;    
//             if(s[i] == '>') ans += (pre[rpos[rp]] - pre[rpos[rp-lsum]] - lsum*(n-i+1)) * 2;
//             else ans += (pre[rpos[rp]] - (rp-lsum-1 >= 0 ? pre[rpos[rp-lsum-1]] : 0) - (lsum+1)*(n-i+1)) * 2;
//             ans += n-i+1;
//         }
...
//     for(int i = 1;i <= n;i++) cin >> a[i];
//     sort(all1(a));
//     int ans = a[n]*2 + a[n-1]*2 - a[1]*2 - a[2]*2;
//     cout << ans << endl;
// }
...
//     f[0] = 0;
//     for(int i = 1;i < N;i++){
//         for(int j = 0;j < 5;j++) if(i-num[j] >= 0){
...
//     while(l <= r){
//         int mid = l + r >> 1;
//         if(y1(mid) > y2(mid)) l = mid + 1;
//         else if(y1(mid) < y2(mid)) r = mid - 1;
//         else {
//             l = r = mid;
//             break;
//         }
...
//     while(l <= r){
//         int mid = l + r >> 1;
//         if(y3(mid) < y2(mid)) r = mid - 1;
//         else if(y3(mid) > y2(mid)) l = mid + 1;
//         else{
//             l = r = mid;
//             break;
//         }
...
//     int mh;
//     for(int i = 0;(1ull<<i) <= m;i++) if((m>>i) & 1){
//         mh = i;
//     }
...
//     vector<int>bit;
//     for(int i = mh+1;(1ull<<i) <= n;i++) if((n>>i) & 1){
//         bit.push_back(i);
//     }
