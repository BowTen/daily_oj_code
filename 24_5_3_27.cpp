...
//	cin >> n;
//	Tree tr(n);
//	vector<int>a(n+5), pre(n+5), last(n+5), suf(n+5, n+1);
//	for(int i = 1;i <= n;i++) cin >> a[i];
//	for(int i = 1;i <= n;i++){
//		pre[i] = last[a[i]];
//		suf[pre[i]] = i;
...
//	int ans = 0;
//	for(int i = 1;i <= n;i++){
//		if(pre[i]) tr.modify(1, 1, n, pre[pre[i]]+1, pre[i], -1);
//		tr.modify(1, 1, n, pre[i]+1, i, 1);
//		if(tr.query(1, 1, n, 1, i) == 0){
//			ans++;
//			if(pre[i]) tr.modify(1, 1, n, pre[pre[i]]+1, pre[i], 1);
//			tr.modify(1, 1, n, pre[i]+1, i, -1);
//			tr.modify(1, 1, n, 1, i, 1);
//			suf[pre[i]] = suf[i];
//			pre[suf[i]] = pre[i];
//			pre[i] = 0;
//		}
...
//		cin >> u >> v >> c >> r;
//		if(r > c){
//			if(c < dis[u]) dis[u] = c, que.push({c, u});
//			if(c < dis[v]) dis[v] = c, que.push({c, v});
//			//vis[u] = vis[v] = 1;	
//			continue;
//		}
//		g[u].push_back({v, c, r});
...
//	for(int i = 29, b;i >= 0;i--){
//		b = (x>>i & 1);
//		if(k>>i & 1) ret = max(ret, val[ne[p][b^1]]);
//		if(!ne[p][b]) break;
//		p = ne[p][b];
//	}
//	return ret;
...
//	for(int i = 1;i <= n;i++){
//		l = max(l, query(a[i]^k));
//		if(last.count(a[i]^k)) r = max(r, last[a[i]^k]);
//		ans += max(0ll, r-l);
//		insert(a[i], i);
//		last[a[i]] = i;
//	}
...
//const int N = 2e5 + 10;
//const int M = N*64;
//int ne[M][2], cnt[M+5], idst[M+5], tp, tot;
//void insert(int x){
//	//cerr << "insert " << x << endl;
//	int p = 0;
//	for(int i = 30;i >= 0;i--){
//		int b = x >> i & 1;
//		if(!ne[p][b]) ne[p][b] = idst[tp--];
//		//if(!ne[p][b]) ne[p][b] = ++tot;
//		p = ne[p][b];
//		cnt[p]++;
//	}
...
//		s[l-1] = 0;
//		for(int i = l;i <= r;i++) s[i] = s[i-1] ^ a[i];
//		int L = mid+1, mx = 0, R = mid;
//		for(int i = mid+1;i <= r;i++){
//			mx = max(mx, a[i]);
//			while(L-1 >= l && a[L-1] <= mx) insert(s[(--L)-1]);
//			ans = max(ans, query(s[i] ^ mx));
//		}
//		while(L <= mid) remove(s[L-1]), L++;

//		s[r+1] = 0;
//		for(int i = r;i >= l;i--) s[i] = a[i] ^ s[i+1];
//		mx = 0;
//		for(int i = mid;i >= l;i--){
//			mx = max(mx, a[i]);
//			while(R+1 <= r && a[R+1] <= mx) insert(s[(++R)+1]);
//			ans = max(ans, query(s[i] ^ mx));
//		}
//		while(R >= mid+1) remove(s[R+1]), R--;
...
//	//for(int i = 1;i <= 10;i++) cerr << idst[M-i] << endl;
//	//insert(5);
//	//for(int i = 1;i <= 10;i++) cerr << idst[M-i] << endl;
//	//remove(5);
//	//for(int i = 1;i <= 10;i++) cerr << idst[M-i] << endl;
...
//int check(int x){
//	int s = query(1, 1, n, 1, x);
//	for(int i = 2;i+x-1 <= n;i++){
//		if(query(1, 1, n, i, i+x-1) != s) return 0;
//	}
//	return 1;
//}
...
//	int n;
//	cin >> n;
//	vector<int>a(n+5), id, b(n+5), pos(n+5);
//	for(int i = 1;i <= n;i++){
//		cin >> a[i];
//		pos[a[i]] = i;
//	}
//	int k = n/2-1, cur = n-k+1;
//	if(pos[1] & 1){
//		for(int i = 2;i <= n-1;i += 2) id.push_back(i);
//	}else{
//		for(int i = 3;i <= n-1;i += 2) id.push_back(i);
//	}
//	sort(all(id), [&](int i, int j) -> int {return a[i] > a[j];});
//	for(auto i : id) b[i] = cur++;

//	id.resize(0);
//	for(int i = 1;i <= n;i++) if(b[i+1] || b[i-1]) id.push_back(i);
//	sort(all(id), [&](int i, int j) -> int {return a[i] > a[j];});
//	cur = 1;
//	for(auto i : id) if(!b[i]){
//		b[i] = cur++;
//	}
